...
public class BlockPlacementPolicyDefault extends BlockPlacementPolicy {
  ...
  private Node chooseTarget(int numOfReplicas,
    ...
    //requiredStorageTypes.size() == totalReplicationExpected
    final List<StorageType> requiredStorageTypes = storagePolicy
    ...
    try {
      if ((numOfReplicas = requiredStorageTypes.size()) == 0) {   //can't not find any feasible storage for any type
      ...
      if (numOfResults == 0) {                                    //we do not choose any storage yet, mean it is a new created block
