...
public class FSDirectory implements Closeable {
  ...
  private long removeLastINode(final INodesInPath iip)
    ...
    if (!parent.removeChild(last, latestSnapshot)) {                  //this is real stuff,the parent tries to remove the child INode
      return -1;
    }
    
    if (!last.isInLatestSnapshot(latestSnapshot)) {                   //if it's not in any snapshot, then update the quota for this path
