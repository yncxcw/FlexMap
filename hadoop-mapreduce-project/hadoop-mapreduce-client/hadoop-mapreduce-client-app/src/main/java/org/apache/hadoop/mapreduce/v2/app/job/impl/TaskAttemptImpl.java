...
public abstract class TaskAttemptImpl implements
  ...
  private void updateProgressSplits() {
    ...
    if (splitsBlock != null) {
      ...
      Counter hdfsRecordsReadCounter = counters.findCounter(TaskCounter.MAP_INPUT_RECORDS);
      if(hdfsRecordsReadCounter != null && hdfsRecordsReadCounter.getValue() <= Integer.MAX_VALUE){
      ...
      if(hdfsRecordsWriteCounter != null && hdfsRecordsWriteCounter.getValue() <= Integer.MAX_VALUE){
    	  ...
    	  splitsBlock.getProgressSpeedHdfsWrite().extend(now,hdfsRecordsWriteCounter.getValue());
    	    	  
      }
      
      Counter fileRecordsReadCounter = counters.findCounter(TaskCounter.REDUCE_SHUFFLE_BYTES);
      if(fileRecordsReadCounter != null && fileRecordsReadCounter.getValue() <= Integer.MAX_VALUE){
    	  
    	  splitsBlock.getProgressSpeedFileRead().extend(now,fileRecordsReadCounter.getValue());
    	    	  
      } 
      
      Counter fileRecordsWriteCounter = counters.findCounter(TaskCounter.MAP_OUTPUT_BYTES);
      if(fileRecordsWriteCounter != null && fileRecordsWriteCounter.getValue() <= Integer.MAX_VALUE){
    	  
    	  splitsBlock.getProgressSpeedFileWrite().extend(now,fileRecordsWriteCounter.getValue());
